...
import { updateUserStart,updateUserFailure, updateUserSuccess, deleteUserStart,deleteUserSuccess,deleteUserFailure, signout } from '../../../redux/nisal/User/userSlice'
...
export default function ManagerProfile() {
  ...
  const handleSubmit=async(e)=>{
    ...
    try{
      ...
      const res=await fetch(`/api/nisal/user/update_manager/${currentUser._id}`,{
        method:'POST',
        headers:{
  ...
  const handledeleteAccount =async ()=>{
    try{
      dispatch(deleteUserStart())
        const res=await fetch(`/api/nisal/user/delete_manager/${currentUser._id}`,{
  ...
  const handleSignOut=async ()=>{
    try{
      ...
      await fetch('api/nisal/auth/manager_signout')
      dispatch(signout())
    }catch(error){
        console.log(error)
    }
  ...
  return (
    ...
    <div className='user-profile'>
      ...
      <div className='flex justify-between mt-5'>
        ...
        <Link className='navigate-button-additem' to='/NAddStaff'><li>Add Shedule</li></Link> 
        <Link className='my-items-button' to='/NStaffDetailsProfile'><li>My Details</li></Link> 
      </div>
