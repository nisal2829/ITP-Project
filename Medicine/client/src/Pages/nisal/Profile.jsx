...
export default function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/nisal/user/update/${currentUser._id}`, {
        method: 'POST',
        headers: {
  ...
  const handledeleteAccount = async () => {
    try {
      ...
      const res = await fetch(`/api/nisal/user/delete/${currentUser._id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success === false) {
  ...
  const handleSignOut = async () => {
    try {
      await fetch('api/nisal/auth/signout');
      dispatch(signout());
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className='user-profile'>
      ...
      <div id='flex justify-between mt-5'>
        ...
        <Link id='navigate-button-addshedule' to='/NAddStaff'>Add Shedule</Link> 
        <Link id='my-details-button' to='/NStaffDetailsProfile'>My Details</Link> 
