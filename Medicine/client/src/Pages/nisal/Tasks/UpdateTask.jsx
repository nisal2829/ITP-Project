...
function UpdateTask() {
  ...
  useEffect(() => {
    const fetchTaskData = async () => {
      try {
        const response = await fetch(`/api/nisal/user/getTask/${id}`);
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(`/api/nisal/user/updateTask`, {
        method: 'PUT',
        headers: {
      ...
      if (data.success) {
        ...
        navigate('/NAllTask'); // Redirect back to AllTask after update
      } else {
        console.error(data.message);
      }
    } catch (error) {
